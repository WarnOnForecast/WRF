...
!     DH%iotype = grid%piofiletype ! pio_iotype_netcdf4p
  end if
...
!       PIO_iotype_netcdf4p = 4     !< netcdf4 (hdf5 format) file opened in parallel
  ...
  stat = pio_openfile(DH%iosystem, DH%file_handle, DH%iotype, FileName, PIO_nowrite)
...
!       PIO_iotype_netcdf4c = 3, &  !< netcdf4 (hdf5 format) file opened for compression (serial write access only)
!       PIO_iotype_netcdf4p = 4     !< netcdf4 (hdf5 format) file opened in parallel
  stat = pio_openfile(DH%iosystem, DH%file_handle, DH%iotype, FileName, PIO_write)
! stat = pio_openfile(DH%iosystem, DH%file_handle, pio_iotype_netcdf, FileName)
! stat = pio_openfile(DH%iosystem, DH%file_handle, pio_iotype_netcdf4p, FileName)
  call netcdf_err(stat,Status)
  if(Status /= WRF_NO_ERR) then
...
!     write(0,*) 'ext_pio_open_for_write_begin: piofiletype = ',grid%piofiletype, DH%iotype 
!     DH%iotype = grid%piofiletype
  
! ERM temporary hack. Not getting grid%piofiletype to work if value is not default in DH
  IF ( grid%piofiletype  /= PIO_iotype_netcdf4p ) THEN
      grid%piofiletype  = PIO_iotype_netcdf4p 
  ENDIF
  IF ( grid%piofiletype /= PIO_iotype_pnetcdf .and. grid%piofiletype /= PIO_iotype_netcdf .and. &
       grid%piofiletype /= PIO_iotype_netcdf4c .and. grid%piofiletype /= PIO_iotype_netcdf4p ) THEN
    write(msg,*) 'Error in ext_pio_open_for_write_begin: Bad value of piofiletype!'
    call wrf_debug ( WARN , TRIM(msg))
    return
  ENDIF

 !call mpi_info_create( info, ierr )
! IF ( grid%piofiletype == pio_iotype_pnetcdf ) THEN
!  stat = pio_CreateFile(DH%iosystem, DH%file_handle, &
!                        grid%piofiletype, FileName, PIO_64BIT_OFFSET)
! ELSE
  stat = pio_CreateFile(DH%iosystem, DH%file_handle, &
                        DH%iotype, FileName, PIO_WRITE)
!                        grid%piofiletype, FileName, PIO_WRITE)
!                        pio_iotype_netcdf4p, FileName, PIO_WRITE)
...
subroutine ext_pio_write_field(DataHandle,DateStr,Var,Field,FieldType,grid, &
  ...
  integer :: deflate, deflate_level, deflate_level_2
  integer :: shuffle
  character*3                :: MemOrd
 ...
 !call pio_setdebuglevel(1)
  ...
  if(len(MemOrd) >= 2) then
    ...
    deflate_level = 2
    deflate_level_2 = 1
    stat = PIO_def_var_deflate(DH%file_handle,DH%descVar(DH%NumVars), shuffle, deflate, &
         deflate_level)
    
     call netcdf_err(stat,Status)
    if(Status /= WRF_NO_ERR) then
       write(0,*) 'deflate failed: stat = ',stat, dh%iotype
     ELSE
 !      write(0,*) 'deflate result: stat = ',stat, dh%iotype,varname
     ENDIF
    ...
    ! perhaps parallel.
    if (DH%iotype .eq. PIO_iotype_netcdf4c .and. stat .ne. PIO_NOERR) then
       write(0,*) "Could not turn on compression for variable foo2222"
       call PIO_closefile(DH%file_handle)
       return
    else if (DH%iotype .eq. PIO_iotype_netcdf4p) then
       !err_msg = "Could not turn on compression for variable foo2222"
       ! if (stat .ne. PIO_NOERR) then
       !    call PIO_closefile(DH%file_handle)
       !    return
       ! end if
       ! return
    else if (DH%iotype .eq. PIO_iotype_pnetcdf .and. stat .eq. PIO_NOERR) then
       write(0,*) "Did not get expected error when trying to turn deflate on for pnetcdf file"
       call PIO_closefile(DH%file_handle)
       return
    else if (DH%iotype .eq. PIO_iotype_netcdf .and. stat .eq. PIO_NOERR) then
       write(0,*) "Did not get expected error when trying to turn deflate on for netcdf classic file"
       call PIO_closefile(DH%file_handle)
       return
    end if
    ENDIF
